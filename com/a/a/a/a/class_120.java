package com.a.a.a.a;

import java.awt.Color;

// $FF: renamed from: com.a.a.a.a.a
public class class_120 {
    // $FF: renamed from: a java.awt.Color
    private static final Color field_494 = new Color(255, 3, 3);
    // $FF: renamed from: b java.awt.Color
    private static final Color field_495 = new Color(0, 66, 255);
    // $FF: renamed from: c java.awt.Color
    private static final Color field_496 = new Color(28, 230, 185);
    // $FF: renamed from: d java.awt.Color
    private static final Color field_497 = new Color(83, 0, 129);
    // $FF: renamed from: e java.awt.Color
    private static final Color field_498 = new Color(255, 252, 1);
    // $FF: renamed from: f java.awt.Color
    private static final Color field_499 = new Color(254, 186, 14);
    // $FF: renamed from: g java.awt.Color
    private static final Color field_500 = new Color(32, 192, 0);
    // $FF: renamed from: h java.awt.Color
    private static final Color field_501 = new Color(229, 91, 176);
    // $FF: renamed from: i java.awt.Color
    private static final Color field_502 = new Color(149, 150, 151);
    // $FF: renamed from: j java.awt.Color
    private static final Color field_503 = new Color(126, 191, 241);
    // $FF: renamed from: k java.awt.Color
    private static final Color field_504 = new Color(16, 98, 70);
    // $FF: renamed from: l java.awt.Color
    private static final Color field_505 = new Color(78, 42, 4);
    // $FF: renamed from: m java.awt.Color
    private static final Color field_506 = new Color(254, 14, 0);
    // $FF: renamed from: n java.awt.Color
    private static final Color field_507 = new Color(254, 121, 3);
    // $FF: renamed from: o java.awt.Color
    private static final Color field_508 = new Color(248, 244, 0);
    // $FF: renamed from: p java.awt.Color
    private static final Color field_509 = new Color(96, 255, 0);
    // $FF: renamed from: q java.awt.Color
    private static final Color field_510 = new Color(2, 201, 179);
    // $FF: renamed from: r java.awt.Color
    private static final Color field_511 = new Color(208, 66, 243);
    // $FF: renamed from: s java.awt.Color[]
    private static final Color[] field_512;

    // $FF: renamed from: a () java.awt.Color
    public static Color method_783() {
        return new Color(field_494.getRGB());
    }

    // $FF: renamed from: b () java.awt.Color
    public static Color method_784() {
        return new Color(field_495.getRGB());
    }

    // $FF: renamed from: c () java.awt.Color
    public static Color method_785() {
        return new Color(field_499.getRGB());
    }

    // $FF: renamed from: d () java.awt.Color
    public static Color method_786() {
        return new Color(field_504.getRGB());
    }

    // $FF: renamed from: a (java.awt.Color, int) java.awt.Color
    public static Color method_787(Color var0, int var1) {
        return new Color(var0.getRed(), var0.getGreen(), var0.getBlue(), var1);
    }

    static {
        field_512 = new Color[]{field_494, field_495, field_496, field_497, field_498, field_499, field_500, field_501, field_502, field_503, field_504, field_505};
    }
}
